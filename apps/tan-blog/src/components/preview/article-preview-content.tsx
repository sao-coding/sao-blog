...
export function ArticlePreviewModalContent() {
  ...
  // TODO(phase5): global overlay, not backed by a single route's
  // validateSearch; strict:false reads the current match's raw search.
  const search = useSearch({ strict: false }) as { 'go-to'?: string }
  const target = useMemo(
    () => parseGoTo(search['go-to'] ?? null),
    [search]
  )
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      getArticlePreview({ data: { type: target!.type, id: target!.id } }),
    enabled: open,
    staleTime: 5 * 60 * 1000,
  ...
  const goToArticle = useCallback(() => {
    // TODO(phase5): target.href is an untyped string path (e.g. `/posts/x`
    // or `/notes/x`); Link/navigate typing wants a literal route id.
    if (target) navigate({ to: target.href })
  }, [navigate, target])
          ...
          {query.isLoading ? (
            ...
            <>
              ...
              <article className="prose dark:prose-invert max-w-full">
                <MdxContent
                  compiledSource={result.serialized.compiledSource}
