...
// 這個元件只會用在 index.tsx 判斷為外部連結（/^https?:\/\//）的 <a>，
// 因此固定用原生 <a>，不是 TanStack Router 的 <Link to>（那個是給站內路由用的）。
    ...
    <Glimpse onOpenChange={(open) => { if (open) setEnabled(true) }}>
      <GlimpseTrigger delay={0} closeDelay={100} render={
        <span className="inline-flex items-center align-top [&_svg]:size-4">
          ...
          <a className="sao-link" href={href} {...props} />
        </span>
      }>
      </GlimpseTrigger>
      <GlimpseContent>
        {isLoading ? (
          ...
          <a className="sao-link block truncate text-sm" href={href}>
            {href}
          </a>
        )}
      </GlimpseContent>
