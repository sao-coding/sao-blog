...
//
// 用 UTC 方法（而非 getMonth/getFullYear/new Date(y,m,d) 這類依賴「執行環境
// 目前時區」的 API）計算，確保 SSR（伺服器時區）與 CSR（瀏覽器時區，可能因
// DevTools 位置模擬或使用者實際所在地而與伺服器不同）算出完全相同的結果，
// 否則 style 裡的 % 位置字串會有些微差異，觸發 hydration mismatch。
...
const seasonSpan = (idx: number, startYear: number): [number, number] => {
  switch (idx) {
    case 0:
      return [Date.UTC(startYear, 2, 1), Date.UTC(startYear, 5, 1)]
    case 1:
      return [Date.UTC(startYear, 5, 1), Date.UTC(startYear, 8, 1)]
    case 2:
      return [Date.UTC(startYear, 8, 1), Date.UTC(startYear, 11, 1)]
    default:
      return [Date.UTC(startYear, 11, 1), Date.UTC(startYear + 1, 2, 1)]
  }
}
